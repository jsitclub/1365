...
def GetData(url, info):
    ...
    tag_list = soup.select(
        "#content > div.content_view > div.service_part > div.board_list.data_middle > ul ")

    if info["cPage"] == 1:
        ## 페이지 갯수 찾기
        num_page = soup.select("#content > div.content_view > div > div.board_bottom > div > div > div")
        global cntPage
        cntPage = (str(num_page).count("<a class=")) + 1
